...
pub fn create_generator_matrix() -> [[u8; 87]; 87] { 
    ...
    pub fn encode174(message: [u8; 10]) -> Option<[u8; 11]> { 
        ...
        let colorder: [usize; 174] =[0,  1,  2,  3, 30,  4,  5,  6,  7,  8,  9, 10, 11, 32, 12, 40, 13, 14, 15, 16,
...
mod tests {
    ...
    use crate::pack::{pack28, pack_grid_or_report, pack75, crc12, pack87};
    ...
    fn pack87_test() {
        assert_eq!(
            ...
            Some([250, 8, 49, 147, 68, 74, 17, 142, 209, 8 | (((2688 & 0xFFF) << 1) >> 8) as u8, ((2688 & 0x0FF) as u8) << 1])
        );
    }
}
